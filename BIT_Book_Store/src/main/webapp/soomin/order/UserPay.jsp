...
<%
	...
	int orderbook_cnt = 0;//바로구매시 넘겨지는 값	
	List<CartBookVO> orderCartList = null;//장바구니 구매시 넘겨지는 값
	
	
	if(request.getAttribute("orderbook_cnt") != null){//바로구매일 때
		orderbook_cnt = (Integer) request.getAttribute("orderbook_cnt");
		
		totalBookPrice = orderBookList.get(0).getBook_saleprice();
		
	}else{//장바구니 구매일 때
		orderCartList = (List<CartBookVO>) request.getAttribute("orderCartList");
		
		for(int c = 0; c < orderCartList.size(); c++) { 
			for(int b = 0; b < orderBookList.size(); b++) {
				if(orderCartList.get(c).getBook_no() == orderBookList.get(b).getBook_no()){
					totalBookPrice += orderBookList.get(b).getBook_saleprice()
							* orderCartList.get(c).getCnt();
				}
			}
		}
...
<style>
	...
	input[name=usepointForPay] {
		text-align: right;
		width: 60px;
		height: 30px;
	}
	
	input[name=usepointForPay]::-webkit-outer-spint-button,
	input[name=usepointForPay]::-webkit-inner-spin-button {
			...
			<div class="row">
			    ...
			    <div class="col-sm-8 order_bar">
			    	<div class="align-left">
						...
						<img class="order-img" alt="장바구니이미지" src="${pageContext.request.contextPath}/resources/images/order/cartImage.png">&nbsp;&nbsp;
						<img class="order-img" alt="사은품이미지" src="${pageContext.request.contextPath}/resources/images/order/goodsImage.png">&nbsp;&nbsp;
						<img class="order-img" alt="결제이미지" src="${pageContext.request.contextPath}/resources/images/order/BuyImage.png">&nbsp;&nbsp;
						<img class="order-img" alt="피니쉬이미지" src="${pageContext.request.contextPath}/resources/images/order/finishBlackImage.png">
					...
					<div class="row">
						...
						<div class="col-sm-7">
							...
							<table class="left_table" border>
								<tr>
									<td class="th">주문자</td>
									<td class="td">${sessionScope.user.user_name } <span class="line">|</span> ${sessionScope.user.phone_number1 } <span class="line">|</span> ${sessionScope.user.email }</td>
								</tr>
							</table>
							<br><br>
								...
								<table class="left_table" border>
									<tr>
										...
										<td>
											...
											<label><input type="checkbox" class="delivery_checkbox"> 기본 배송지와 동일</label>
										</td>
									</tr>
									<tr>
										<td>휴대폰</td>
									...
									<tr>
										...
										<td>
											<div style="line-height: 220%;">
												...
												<label><input type="checkbox" name="basic_addr" value="1">기본 배송지로 저장</label>
												<br>
							...
							<div id="tab2" class="tabcontent">
								<table class="left_table" border>
									...
									<tr>
										...
										<td>
											<div style="line-height: 220%;">
												<input id="inputzip2" type="text" name="order_recv_zip_code" readonly>&nbsp;
												<button type="button" class="btn btn-secondary" id="find_addr2" onclick="find_addr()">주소 찾기</button>
												<label><input type="checkbox" name="basic_addr">기본 배송지로 저장</label>
												<br>
												<input class="inputaddr" type="text" name="order_recv_street_addr"><br>
												<input class="inputaddr" type="text" name="order_recv_remaining_addr">
											</div>
							...
							<div class="margin_top50">
								<div class="box_title_area">
									...
									<span  style="float: right;">
										<a href="javascript:;" class="btn_toggle on">
											<img class="btn_toggle_img" alt="열기/닫기" src="${pageContext.request.contextPath}/resources/images/order/top.png">
										</a>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									</span>
								</div>
								<div style="display: block;">
									<table class="left_table" border>
										...
										<c:forEach var="cartbookvo" items="${requestScope.orderCartList }" varStatus="status">
										<tr>
											<td>
											...
											<c:if test="${status.first }">
												<td class="center" rowspan="${requestScope.orderCartList.size() + requestScope.orderGoodsVOList.size() }">
													<c:if test="${requestScope.store_code eq 'online' }">
													온라인배송
													</c:if>
													<c:if test="${requestScope.store_code ne 'online' }">
										...
										<tr>
											...
											<td class="center middle">
												<c:if test="${not empty orderbook_cnt }"><!-- 바로구매일 때 -->
													${bookvo.book_saleprice * orderbook_cnt }원 <span class="line">|</span> 수량 ${orderbook_cnt }
												</c:if>
												<c:if test="${empty orderbook_cnt }"><!-- 장바구니에서 구매일 때 -->
													<!-- bookvo의 book_no과 cartvo들의 book_no을 비교해서 일치하는 것의 가격을 출력 -->
													<c:forEach var="cartvo" items="${requestScope.orderCartList }">
														<c:if test="${cartvo.book_no eq bookvo.book_no}">
															${bookvo.book_saleprice * cartvo.cnt } <span class="line">|</span> 수량 ${cartvo.cnt }
														</c:if>
													</c:forEach>
												</c:if>
												개<br>
											...
											<c:if test="${status.first }">
												<td class="center" rowspan="${requestScope.orderBookList.size() + requestScope.orderGoodsVOList.size() }">온라인배송</td>
											</c:if>
										</tr>
										</c:forEach> --%>
										...
										<tr>
											<td>
												<img class="book-img" alt="굿즈 이미지${goodsvo.goods_code }" src="${pageContext.request.contextPath}/resources/images/goods_img/${goodsvo.goods_image }">
								...
								<table class="left_table pay-table">
									<tr>
										<td class="center pay-table payImg" id="kakaopay">
											<img alt="카카오페이" src="${pageContext.request.contextPath}/resources/images/pic/kakaopay.png">
										</td>
										<td class="center pay-table payImg" id="smilepay">
											<img alt="스마일페이" src="${pageContext.request.contextPath}/resources/images/pic/smilepay.png">
										</td>
									</tr>
								</table>
							...
							<div class="margin_top50">
								<table class="left_table right_content" border style="border: 1px solid black;">
									<tr>
										...
										<td class="text-align-right">${pageScope.totalBookPrice } 원</td>
									</tr>
									<tr>
										<td>사은품 구매 금액</td>
